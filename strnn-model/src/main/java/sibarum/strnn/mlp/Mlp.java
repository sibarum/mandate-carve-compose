package sibarum.strnn.mlp;

import java.util.Arrays;
import java.util.Random;

/**
 * From-scratch tiny MLP: any number of fully-connected layers, ReLU hidden,
 * linear output. MSE loss against a target during backward(). Plain SGD on
 * step(). Caches activations from the most recent forward pass so backward()
...
public final class Mlp {
    ...
    /**
     * Computes gradients given a target output, accumulating into gradWeights /
     * gradBiases. Returns dInput so the caller can chain gradients further back
     * if needed (not used in v0 — the only learnable parameters are inside
     * MLPs, and they sit at the leaves of the differentiable subgraph).
     */
