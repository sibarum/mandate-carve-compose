...
/**
 * Split-quaternion algebra on 4-vectors in the basis {1, i, j, k} with
 * relations:
 *
 * <pre>
 *   i² = -1
 ...
 * </pre>
 *
 * Norm form: N(a) = a_0² + a_1² - a_2² - a_3² (signature (++--)). The
 * canonical idempotents are e_+ = (1/2, 0, 1/2, 0) and e_- = (1/2, 0, -1/2, 0).
 * Both lie on the null cone {N = 0}.
 */
public final class SplitQuat {
    ...
    /**
     * Full split-quaternion product (a · b) in 4-vector form. See the
     * multiplication table at the top of the file.
     */
    ...
    /** Split-quaternion conjugate: scalar part unchanged, i/j/k parts negated. */
    public static double[] conj(double[] a) {
        return new double[] { a[0], -a[1], -a[2], -a[3] };
    }

    /**
     * Backward of c = a · b w.r.t. the left operand. Given dL/dc, returns
     * dL/da treating b as held fixed. Derived by reading the Jacobian off
    ...
    /** Backward of c = a · b w.r.t. the right operand. */
    ...
    /**
     * Conjugate sandwich: y = q · v · q̄. The split-quaternion analog of
     * a rotation by a unit quaternion — except split-quat norms aren't
     * positive-definite, so the sandwich rescales by N(q) on the unit
     * cells and stretches/compresses in the hyperbolic directions.
     */
    ...
    /**
     * Backward of y = q · v · q̄. Both occurrences of q contribute to dL/dq;
     * sum the two paths. Returns {dQ, dV}.
     */
