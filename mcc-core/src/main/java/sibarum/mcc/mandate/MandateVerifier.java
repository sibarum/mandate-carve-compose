...
/**
 * Structural verifier. After {@link ComputationGraph#execute} has
 * populated each node's produced value, {@link #verify} checks each
 * mandate against every node and enforces ordering constraints via
 * topological reachability.
 *
 * <p>"Somewhere" is implemented literally: any node in the executed
 * DAG counts. The result mandate is exempt — it is matched
 * specifically against the terminal node, since that is the value the
