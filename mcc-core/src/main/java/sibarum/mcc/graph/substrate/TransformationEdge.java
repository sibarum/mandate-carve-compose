...
/**
 * Directed edge in the transformation graph. Exists when {@code from}'s
 * output type appears among {@code to}'s input types. Slot selection
 * at the target is deferred to graph-instantiation time; one
 * {@code TransformationEdge} can serve any of the type-compatible
 * slots of the target node.
 */
public final class TransformationEdge {
    ...
    public TransformationNode from() { return from; }
    public TransformationNode to() { return to; }
    public EdgeStats stats() { return stats; }
