...
/**
 * Polynomial (Π-net) lift: maps a vector {@code k ∈ ℝⁿ} to its vector
 * of all degree-{@code d} monomials. For {@code (n=4, d=2)} that's 10
 * monomials including cross-terms ({@code k₀k₁}, {@code k₀k₂}, …); for
 * general {@code n} and {@code d} the count is {@code C(n + d − 1, d)}
 * (multiset coefficient).
 *
 * <p>The lift itself is parameter-free; a separate learned projection
 * typically takes the monomial vector down to a working dimension.
 *
 * <p>Convention for monomial enumeration: lex order on the multi-index
 * {@code (α₀, α₁, …, α_{n-1})} with {@code Σαᵢ = d}. Highest
 * {@code α₀} first. Reproducible across runs, so any gradient-trained
 * downstream projection matches the same row order every time.
 */
public final class PolyLift {

    private PolyLift() {}

    /** Multiset coefficient {@code C(n + d − 1, d)}. */
    ...
    /**
     * Enumerate all multi-indices {@code (α₀, …, α_{n-1})} of length
     * {@code n} summing to {@code d}. Each returned array is fresh.
     * Lex order, {@code α₀} first.
     */
    ...
    /**
     * Evaluate every monomial in the list at {@code k}. Output length
     * equals {@code monomials.length}.
     */
    ...
    /**
     * Backward through the lift. Given {@code dL/dm} (gradient on the
     * monomial vector) and the input {@code k} at which the lift was
     * evaluated, returns {@code dL/dk}.
     *
     * <p>{@code ∂M_α/∂k_j = α_j · k_j^{α_j − 1} · ∏_{l ≠ j} k_l^{α_l}},
     * with the {@code α_j = 0} case yielding 0.
     */
