 ...
 *
 * <p>Designed as a drop-in alternative to {@link Mlp} for tasks of
 * shape {@code (Din,)} per token, {@code S} tokens per example,
 * {@code Dout} scalar output. Layer norm is omitted to keep the
 * implementation small; residual connections are present around both
 * the attention and FFN sub-blocks.
 *
 * <p>Loss: MSE against a target. {@link #backward} accumulates
 * gradients; {@link #step} applies them.
 *
 * <p>{@link TransformerBlock} wraps this as a Trainable Primitive for
 * graph use.
 */
public final class Transformer {
    ...
    public int inputDim()  { return seqLen * dIn; }
    public int outputDim() { return dOut; }
    ...
    public double[] forward(double[] flatInput) {
        ...
        attnOut = matmul(attn, V);
        for (int i = 0; i < seqLen; i++) {
            for (int k = 0; k < dModel; k++) attnOut[i][k] += xp[i][k];
        }

        hiddenPre = addRowVec(matmul(attnOut, W1), b1);
        hidden = relu(hiddenPre);
        ffnOut = addRowVec(matmul(hidden, W2), b2);
        for (int i = 0; i < seqLen; i++) {
